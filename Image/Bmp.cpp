...
BITMAPINFO *lpBitsInfo = NULL; // bmp的info信息+调色板+实际位图数据
...
void getPixelStr(int i, int j, char *str)
{
	if (NULL == lpBitsInfo)
		return;
...
void pixel(int i, int j)
{
	...
	if(strcmp(str,"")) AfxMessageBox(str);
}
...
BITMAPINFO* Create_FFT_Image(complex<double>* FD)
{
	...
	//为频域图像分配内存
	LONG size = 40 + 1024 + (width * 8 + 31) / 32 * 4 * height;
...
BITMAPINFO* Create_IFFT_Image(complex<double>* FD)
{
	...
	if (NULL == new_lpDIB)
		return NULL;
	memcpy(new_lpDIB, lpBitsInfo, size);
